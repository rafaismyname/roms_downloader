...
# docker run --rm -it --platform=linux/arm64 -v "$PWD":"$PWD" -w "$PWD" ubuntu:20.04 ./linux/build-arm64.sh

set -euo pipefail

echo "[*] Starting ARM64 build script"
echo "[*] CWD: $(pwd)"
echo "[*] uname: $(uname -a)"

REPO_ROOT="$(pwd)"
BUILD_ROOT="${REPO_ROOT}/.arm64_build"
...
# ---------------------------------------------------------------------------
# 5. Prepare dist layout under .arm64_build
# ---------------------------------------------------------------------------
...
cp "${REPO_ROOT}/linux/roms_downloader.sh" "${DIST_ROOT}/ports/roms_downloader.sh"
chmod +x "${DIST_ROOT}/ports/roms_downloader.sh"
chmod +x "${PORT_ROOT}/flutter-pi"

echo "[*] Creating zip inside ${BUILD_ROOT}..."
cd "${DIST_ROOT}"
ZIP_PATH="${BUILD_ROOT}/roms_downloader_portmaster.zip"
rm -f "${ZIP_PATH}"
